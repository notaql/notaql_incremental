...
import notaql.performance_tests.hbase.copy_inserts.PerformanceTestHbaseIncrementalTimestampioCopyInserts;

/**
 * Tests a simple SUM() with additionaly inserted data
 */
public class PerformanceTestHbaseIncrementalTimestampioSumInserts extends PerformanceTestHbaseIncrementalTimestamp {
	/* (non-Javadoc)
	...
	public void test() throws Exception {
		...
		// 1. Execution
		notaqlEvaluate(PerformanceTestHbaseIncrementalTimestampioCopyInserts.generateQuery(query, false));
		...
		// 2. Execution
		notaqlEvaluate(PerformanceTestHbaseIncrementalTimestampioCopyInserts.generateQuery(query, true));
		logCurrentTimestampToFile("Execution 2");
	}
	
	
	public static void main(String[] args) throws Exception {
		(new PerformanceTestHbaseIncrementalTimestampioSumInserts()).execute();
	}
}
