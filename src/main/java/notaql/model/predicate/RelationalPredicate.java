...
public class RelationalPredicate implements Predicate {
    ...
    private boolean compare(List<Value> leftValues, List<Value> rightValues) {
        ...
        final List<Comparable<?>> comparableValues = new ArrayList<>(values.size());

        if (scalar instanceof Comparable<?>) {
            	values.stream()
                    .filter(v -> v instanceof Comparable<?> &&
                    		(v.getClass().equals(scalar.getClass())
                    		 || (scalar instanceof DeltaValue 
                    			&& v.getClass().equals(scalar.getClass().getSuperclass()
            					)
                			 )))
