    ...
    public VData visitAggregateVData(@NotNull NotaQL2Parser.AggregateVDataContext ctx) {
        ...
        switch (aggrCtx.function.getType()) {
            ...
            case NotaQL2Parser.COUNT:
                try {
                	return new CountVData(visit(aggrCtx.vData()));
                } catch (NullPointerException e) {
                	throw new EvaluationException("COUNT() needs one argument (e.g. COUNT(1) will count each element). For CDC this argument has to point at the changing data (e.g. the row-id)."); // CDC doesn't work without delta-flags (preserved, inserted, deleted). Flags are stored together with the values.
                }
